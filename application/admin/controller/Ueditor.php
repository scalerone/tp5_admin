...
class Ueditor extends AdminController {
	...
	function upload(){
	    ...
	    $CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents("./static/admin/js/ueditor/config.json")), true);
	    $action = $_GET['action'];
...
// 	    var_dump($files);die;
		...
		if ($files) {
			...
			$state = 'SUCCESS';
			$file = $path."ueditor/".$result['savename'];
...
// 		$path = 'upload'; //最好使用缩略图地址，否则当网速慢时可能会造成严重的延时
		$path = config('img_path'); //最好使用缩略图地址，否则当网速慢时可能会造成严重的延时
	...
	public function get_remote_image(){
		...
		//远程抓取图片配置
		$config = array(
				"savePath" => './'.config('img_path')."ueditor/",            //保存路径
	...
	private function _get_remote_image(){
		...
		//远程抓取图片配置
		$config = array(
				"savePath" => './'. config('img_path')."ueditor/$date/",            //保存路径
				"allowFiles" => array( ".gif" , ".png" , ".jpg" , ".jpeg" , ".bmp" ) , //文件允许格式
				"maxSize" => 3000                    //文件大小限制，单位KB
		);
		...
		foreach ( $source as $imgUrl ) {
			...
			//写入文件
			$tmpName = $savePath .$file ;
			$file = config('img_path')."ueditor/$date/".$file;
			if(strpos($file, "https")===0 || strpos($file, "http")===0){
	...
	private function _ueditor_upload($config=array()){
		...
		if ($files && is_array($files)) {
		    foreach ($files as $file) {
		        ...
		        $result[] = [
		            ...
		            'size'		=>		$info->getSize(),
		            'savePath'	=>		$date,
		            'saveName'	=>		$date . '/' . $info->getFilename() ,
		        ] ;
		    }
		}
		...
		if ($files) {
			...
			//上传成功
			$title = $oriName = $result[0]['name'];
			$size=$info->getSize();
		
			$state = 'SUCCESS';
			
			$url = $result[0]['saveName'];
