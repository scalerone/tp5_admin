        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
         ...
         <form id="searchForm" class="form-horizontal adminuserform">
            <div class="col-md-2 col-sm-12 col-xs-12 form-group">
            	<?php if((checkPopedom('/admin/adminuser/add'))): ?>
                <a href="<?php echo url('/admin/adminuser/add'); ?>" class="btn btn-sm btn-success" type="button"><i class="fa fa-plus-square"></i> 新增</a>
                <?php endif; ?>
            </div>
            <div class="input-group col-md-3 col-sm-4 col-xs-12 form-group pull-right">
                <input type="text" name="uname" class="form-control form-param-to" placeholder="请输入用户名" value="<?php echo $param['uname']; ?>">
            ...
            <div class="input-group col-md-2 col-sm-4 col-xs-12 form-group pull-right">
                <input type="text" name="mobile" class="form-control form-param-to" placeholder="请输入手机号" value="<?php echo $param['mobile']; ?>">
            </div>
                        ...
                        <tbody>
                        	 <?php if(is_array($page->items()) || $page->items() instanceof \think\Collection): $i = 0; $__LIST__ = $page->items();if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
				                 <tr class="even pointer">
									<!-- <td class="a-center ">
									    <input type="checkbox" class="flat" name="table_records">
									</td> -->
									<td class=" "><?php echo $vo['userId']; ?></td>
									<td class=" "><?php echo $vo['userName']; ?></td>
				                     <td class=" "><?php echo $vo['realName']; ?></td>
				                     <td class=" "><?php echo $vo['mobile']; ?></td>
				                     <td class=" "><?php echo $vo['inviterUserName']; ?></td>
				                     <td class=" "><?php if(($vo['approved']==1)): ?>是<?php else: ?>否<?php endif; ?></td>
				                     <td class="a-right a-right "><?php echo $vo['Createtime']; ?></td>
				                     <td class="a-right a-right "><?php echo $vo['lastLoginDate']; ?></td>
									<td class=" last">
								<?php if((checkPopedom('/admin/adminuser/info'))): ?>
                            <a href="<?php echo url('/admin/adminuser/info',['id'=>$vo['userId']]); ?>" class="btn btn-primary btn-xs"><i class="fa fa-folder"></i> 查看</a>
                            	<?php endif; if((checkPopedom('/admin/adminuser/edit'))): ?>
                            <a href="<?php echo url('/admin/adminuser/edit',['id'=>$vo['userId']]); ?>" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> 编辑</a>
                           		<?php endif; if((checkPopedom('/admin/adminuser/delAjax'))): ?>
                            <a href="javascript:void(0);" data-href="<?php echo url('/admin/adminuser/delAjax',['id'=>$vo['userId']]); ?>"  class="btn btn-danger btn-xs js-ajax-delete"  ><i class="fa fa-trash-o"></i> 删除 </a>
                        		<?php endif; ?>
                        		<br>
                           		<?php if((checkPopedom('/admin/adminuser/changeAjax'))): if(($vo['approved']===1)): ?>
                            <a href="javascript:void(0);" class="btn btn-warning btn-xs btn-change-approved" id_val="<?php echo $vo['userId']; ?>" st_val="0"><i class="fa fa-pencil"></i> 禁用</a>
                            <?php else: ?>
                            <a href="javascript:void(0);" class="btn btn-success btn-xs btn-change-approved" id_val="<?php echo $vo['userId']; ?>" st_val="1"><i class="fa fa-pencil"></i> 启用</a>
                            <?php endif; endif; if((checkPopedom('/admin/adminuser/changeLockAjax'))): if(($vo['is_lock']===1)): ?>
                            <a href="javascript:void(0);" class="btn btn-success btn-xs btn-change-lock" id_val="<?php echo $vo['userId']; ?>" st_val="0" ><i class="fa fa-unlock-alt"></i> 解锁 </a>
                        	<?php endif; endif; ?>
									</td>
								</tr>
								<?php $sindex = 'sindex++'; endforeach; endif; else: echo "" ;endif; ?>
                        </tbody>
                        </table>
                    </div>
		...
		if(id_val>0){
			...
			layer.confirm('确认要更改吗？', {
				...
				title:'提示',
  				...
  				layer.close(index);
  				$.getJSON("<?php echo url('/admin/adminuser/changeAjax'); ?>",{id:id_val,st:st_val},
  				function(data){
		...
		if(id_val>0){
			...
			layer.confirm('确认要更改吗？', {
				...
				title:'提示',
  				...
  				layer.close(index);
  				$.getJSON("<?php echo url('/admin/adminuser/changeLockAjax'); ?>",{id:id_val,st:st_val},
  				function(data){
  					if(null != data.msg || '' != data.msg){
  						layer.msg(data.msg);
  					}
